...
public class SaTokenRedisConfiguration {
	...
	protected RedisConnectionFactory redisConnectionFactory() {
		...
		if (alone) {
			...
			// 单机模式/集群模式
			if (aloneRedis.getCluster() == null || CollectionUtils.isEmpty(aloneRedis.getCluster().getNodes())) {
				...
				List<RedisNode> redisNodes = cluster.getNodes().stream().map(node -> {
					String[] hostAndPorts = StringUtils.split(node, ':');
