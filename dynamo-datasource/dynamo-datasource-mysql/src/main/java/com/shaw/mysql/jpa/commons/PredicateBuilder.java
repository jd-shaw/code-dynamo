...
import com.shaw.mysql.jpa.commons.QueryBetween;
...
public class PredicateBuilder<T> {
	...
	protected Predicate getPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb, Query queryParam) {
		...
		for (Field field : fields) {
			...
			if (ObjectUtils.isEmpty(value) && queryType.isNotCanBeNull()) {
	...
	private void buildPredicate(QueryType queryType, List<Predicate> predicates, String queryFiled, Object value) {
		...
		case BEWTEEN:
			...
			if (ObjectUtils.isNotEmpty(predicate)) {
	...
	protected Path<Object> getRootByQueryFiled(String queryFiled, Root<T> root) {
		if (!queryFiled.contains(POINT_MARK)) {
			return root.get(queryFiled);
		} else {
			return getRootByQueryFiled(queryFiled.substring(queryFiled.indexOf(".") + 1),
	...
	protected Path<String> getRootByQueryFiledString(String queryFiled, Root<T> root) {
		if (!queryFiled.contains(POINT_MARK)) {
			...
			return getRootByQueryFiledString(queryFiled.substring(queryFiled.indexOf(".") + 1),
					root.get(queryFiled.substring(0, queryFiled.indexOf("."))));
		}
	}
	...
	protected Path<Comparable> getRootByQueryFiledComparable(String queryFiled, Root<T> root) {
		if (!queryFiled.contains(POINT_MARK)) {
			...
			return getRootByQueryFiledComparable(queryFiled.substring(queryFiled.indexOf(".") + 1),
					root.get(queryFiled.substring(0, queryFiled.indexOf("."))));
		}
	}
