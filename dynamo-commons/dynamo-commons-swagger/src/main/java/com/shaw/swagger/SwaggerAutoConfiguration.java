...
public class SwaggerAutoConfiguration implements BeanDefinitionRegistryPostProcessor, EnvironmentAware {
	...
	private static final String SWAGGER_PROPERTIES_PREFIX = "dynamo.common.swagger";
	 ...
	 */
	@Bean
	@ConditionalOnProperty(prefix = "dynamo.common.swagger", value = "enabled", havingValue = "true", matchIfMissing = true)
	...
	public void postProcessBeanDefinitionRegistry(@NotNull BeanDefinitionRegistry registry) throws BeansException {
		if (swaggerProperties.isEnabled()) {
			Map<String, List<String>> basePackages = this.swaggerProperties.getBasePackages();
			AtomicInteger atomicInteger = new AtomicInteger(96);
			basePackages.forEach((name, basePackage) -> {
				String[] packages = basePackage.toArray(new String[basePackage.size()]);
	...
	public void setEnvironment(@NotNull Environment environment) {
		BindResult<SwaggerProperties> bind = Binder.get(environment).bind(SWAGGER_PROPERTIES_PREFIX,
